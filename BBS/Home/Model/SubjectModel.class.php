...
class SubjectModel extends Model{
	...
	public function pre_dofollow(){
		...
		if($newdata){
				...
				//=============实例化subjiect类 将最回复人id 回复时间 楼层进行修改===========================================
				//=============实例化users类 将最回复人积分进行修改===========================================
				$user=D('users');
				$map1=[];
				$map1['id']=['eq',$data['uid']];
				$usersList=$user->where($map1)->select();
				//dump($usersList);
				$udata['integral']=$usersList['0']['integral']+1;//每发一个回复积分加1
				//dump($udata);
				$user->where($map1)->save($udata);
				//=============实例化users类 将最回复人积分进行修改===========================================				//=============调用everyday函数 将当天的回复数量进行修改===========================================
				Everyday('follow_num');
